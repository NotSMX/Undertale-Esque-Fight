...
@onready var hurt_sound: AudioStreamPlayer = get_node(hurt_sound_path)
@onready var dust_sound: AudioStreamPlayer = get_node(dust_sound_path)
@onready var dust: GPUParticles2D = get_node(dust_path)
@onready var sprites: Node = get_node(sprites_path)

## Set once by whatever spawns this enemy (e.g. your battle setup script), before it takes a turn.
var Box: BattleBox
var Soul: SoulBattle
